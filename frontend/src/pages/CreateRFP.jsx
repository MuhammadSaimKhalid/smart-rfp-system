...
import { ArrowLeft, Save, Send, Sparkles, CheckCircle, AlertCircle, FileText, Download, Upload } from 'lucide-react';
...
export default function CreateRFP() {
    ...
    const [rfpData, setRfpData] = useState({
        ...
        budget: "",       // Required
        status: "draft",
        proposal_form_rows: [], // Extracted form rows
        proposal_form_schema: {} // Extracted structure info
    });
    ...
    const handleFileUpload = async (event) => {
        ...
        try {
            ...
            setRfpData({
                ...
                status: "draft",
                proposal_form_rows: extracted.proposal_form_rows || [],
                proposal_form_schema: extracted.proposal_form_schema || {}
            });
        ...
        <div className="flex flex-col h-screen overflow-hidden bg-slate-50">
            ...
            <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 flex-shrink-0 z-20 shadow-sm">
                ...
                <div className="flex gap-3">
                    {rfpData.id && (
                        <button
                            onClick={() => window.open(`http://localhost:8000/api/rfps/${rfpData.id}/pdf`, '_blank')}
                            className="text-sm font-medium text-slate-500 hover:text-slate-800 px-3 flex items-center gap-2"
                            title="Download RFP as PDF"
                        >
                ...
                <div className="h-full overflow-y-auto bg-slate-100 p-8 flex justify-center">
                    <div className="w-[850px] min-h-[1100px] bg-white shadow-sm border border-slate-200 p-12 transition-all">
                        ...
                        <div className="space-y-10">
                            ...
                            {rfpData.proposal_form_rows && rfpData.proposal_form_rows.length > 0 && (
                                <section className="break-inside-avoid">
                                    <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wide border-b border-slate-200 pb-2 mb-4 flex items-center gap-2">
                                        5. Proposal Submission Form <span className="text-green-600" title="Auto-Generated">✨</span>
                                    </h3>
                                        ...
                                        <table className="w-full text-sm text-left border-collapse">
                                            <thead className="text-xs bg-slate-50 uppercase text-slate-500 font-semibold">
                                                <tr>
                                                    ...
                                                    {/* Dynamically render vendor columns if available in schema, otherwise defaults */}
                                                    {rfpData.proposal_form_schema?.vendor_columns?.map((col, i) => (
                                                        <th key={i} className="px-3 py-2 border border-slate-200">{col}</th>
                                                    )) || (
                                                            <>
                                            ...
                                            <tbody>
                                                {rfpData.proposal_form_rows.reduce((acc, row, idx) => {
                                                    // Insert Section Header if section changes
                                                    const prevSection = idx > 0 ? rfpData.proposal_form_rows[idx - 1].section : null;
                                                    ...
                                                    acc.push(
                                                        <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50">
                                                            ...
                                                            {/* Render empty cells for vendor columns to simulate the form */}
                                                            {(rfpData.proposal_form_schema?.vendor_columns?.length ? rfpData.proposal_form_schema.vendor_columns : ['Qty', 'Unit', 'Cost', 'Total']).map((_, i) => (
                                                                <td key={i} className="px-3 py-2 border border-slate-200 bg-slate-50 text-slate-500 text-xs">
                                                                    {/* If row has pre-filled values matching common keys, show them */}
                                                                    {i === 0 && row.quantity ? row.quantity : ''}
                                                                    {i === 1 && row.unit ? row.unit : ''}
                                                                </td>
                                                            ))}
                                                        </tr>
                                                    );
